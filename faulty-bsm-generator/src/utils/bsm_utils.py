...
def get_coords_outside_region(code_str, margin_deg=5):
    ...
    lon = int(np.random.uniform(COUNTRY_BOUNDS[840]["min_lon"] - 1000, COUNTRY_BOUNDS[840]["min_lon"]))
    lat = int(np.random.uniform(COUNTRY_BOUNDS[840]["max_lat"], COUNTRY_BOUNDS[840]["max_lat"] + 1000))

    #print("bounds:", country_geom.bounds)
    #pts = None
    # while pts is None:
    #     lon = int(np.random.uniform(COUNTRY_BOUNDS["min_lon"] - 1000, COUNTRY_BOUNDS["min_lon"]))
    #     lat = int(np.random.uniform(COUNTRY_BOUNDS["max_lat"], COUNTRY_BOUNDS["max_lat"] + 1000))
